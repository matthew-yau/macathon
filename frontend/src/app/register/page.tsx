    ...
    try {
      ...
      if (existingFaculty) {
        ...
        if (insertFacultyError) {
          ...
          throw new Error(
            `Error creating faculty: ${insertFacultyError.message}`
          );
        }
      ...
      if (existingIntention) {
        ...
        const { data: newIntention, error: insertIntentionError } =
          await supabase
            .from("intentions")
        ...
        if (insertIntentionError) {
          ...
          throw new Error(
            `Error creating intention: ${insertIntentionError.message}`
          );
        }
      ...
      for (const { prompt: promptText, answer } of selectedPrompts) {
        ...
        if (existingPrompt) {
          ...
          if (insertPromptError) {
            console.error(
              `Error creating prompt ${promptText}:`,
              insertPromptError
            );
            continue;
          }
      ...
      // Step 7: Upload profile pictures and insert URLs into user_photos
      ...
      if (validPhotos.length > 0) {
        for (const photo of validPhotos) {
          ...
          const { error: uploadError } = await supabase.storage
            .from("upload") // Replace with your actual bucket name
          ...
          const { error: insertPhotoError } = await supabase
            .from("user_photos")
            .insert({ user_email: email, photo_url: publicUrl });

          if (insertPhotoError) {
            console.error(
              "Error inserting photo URL:",
              insertPhotoError.message
            );
          }
        }
      }
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <div className="rounded-md bg-pink-50 p-4">
            <h2 className="mb-4 text-xl font-semibold text-pink-800">
              ...
              <span className="ml-2 text-sm font-normal text-pink-700">
                (Fill all three)
              </span>
            </h2>
                ...
                <div key={index} className="space-y-2">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updatePromptAndAnswer(index, "prompt", e.target.value)
                      }
                      required
                      className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none"
                    >
                      ...
                      {promptOptions.map((prompt) => (
                        <option
                          key={prompt}
                          value={prompt}
                          disabled={selectedPrompts.some(
                            (p, i) => i !== index && p.prompt === prompt
                          )}
                        >
                          {prompt}
                        </option>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updatePromptAndAnswer(index, "answer", e.target.value)
                      }
                      required
                      className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none"
                      rows={3}
                  ...
                  <div key={index} className="rounded-md bg-indigo-50 p-4 mt-8">
                    <h2 className="mb-2 text-xl font-semibold text-indigo-800">
                      Upload Profile Picture {index + 1}
                    </h2>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={handleIndexedFileChange(index)}
                      className="block w-full text-sm text-gray-500 file:mr-4 file:rounded-md file:border-0 file:bg-indigo-600 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-indigo-700"
                    />
                  </div>
                </div>
              ))}
